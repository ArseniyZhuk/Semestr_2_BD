...
WITH slots AS (
                SELECT facility, SUM(b.slots) AS Стоимость_аренды,   
...
ROUND(sl1.Стоимость_аренды / SUM(sl2.Стоимость_аренды) * 100, 1) AS Процент_использования_объектов_в_месяц, sl1.Год_Месяц
FROM slots AS sl1
